...
> 原本: `raw-data/Minimax audioプロンプト.txt`  
> モデル: `speech-2.5-hd-preview`  
> 基本Emotion: `Neutral` または `Auto`

Hailuo Audio（MiniMax TTS）では `<#秒数#>` 構文で沈黙（間）を挿入できる。感情演出は Emotion パラメータで制御する。

#### 口喧嘩シーン（鈴木 vs 主人公）— Angry / Disgusted

| Emotion | 入力テキスト | 備考 |
|---|---|---|
| Disgusted | `華族もろとも学校の笑いものだったぞ。` | 「家族」を「華族」にしたのは「いえぞく」誤読を防ぐカタカナ/漢字工夫 |
| Disgusted | `どの口が言ってんだ？。高校を辞めてお前はどうなった。` | |
| Angry | `もうお前は佐藤に手コキさせてろ！。` | Disgustedでは怒りが弱すぎたためAngryに変更 |
| Angry | `誰でもいいだろ。` | |
| Disgusted | `ガキが御託並べんな。将来も考えずに自堕落に生きてるバカとつるむ気なんかない。` | |
| Disgusted | `もうほっといてくれ。俺は仕事で手一杯だ。他のやつのことなんか知るか。` | |
| Disgusted | `だから知らないって言ってるだろ。` | |
| Disgusted | `誰だよ。` | |
| Angry | `知らないよ。<#1.0#>今何時だと思ってるんだ。夜中の３時だぞ。` | `<#1.0#>` で1秒の間を挿入 |

#### 主人公の反論 — Sad

| Emotion | 入力テキスト | 備考 |
|---|---|---|
| Sad | `<#2.9#>自分に関係ないと切り捨てて不安をごまかしてるだけだろ。<#3.0#>` | 冒頭2.9秒・末尾3.0秒の間を計算して挿入 |
| Sad | `<#1.9#>あんた空っぽだな。ただの社会の傀儡か。<#5.0#>` | Angryではなく**Sad**で抑制された怒りを表現 |

#### 佐藤との感情シーン — Sad

| Emotion | 入力テキスト | 備考 |
|---|---|---|
| Sad | `佐藤？。<#2.0#>どうした？。大丈夫か？` | `<#2.0#>` で2秒の間 |
| Sad | `ごめん。<#1.3#>何か話してくれないか？。<#1.0#>なんでもいいから。<#4.0#>` | 複数の間で途切れ途切れの感情表現 |
| Sad | `全部ぼくのせいだ。<#3.1#>ありがとう。<#2.5#>君に触れたい。<#2.0#>` | 秒数を細かく設計 |

#### ニュートラル・その他

| Emotion | 入力テキスト | 備考 |
|---|---|---|
| Neutral | `くがつ二十いちにちです。` | AI佐藤の台詞。「九月」をひらがな化 |
| Auto | `そちらで購入したAI『佐藤』の調子が悪いのですが。<#7.0#>` | カスタマーサポートへの電話。7秒の長い間 |
| Disgust | `孤独なまま社会から捨てられて` | 主人公の独白 |
| — | `え、事件がくがつ二十一日か…。` | 「九月」→「くがつ」の漢字→平仮名変換の実例 |
| Neutral | `<#3.0#>佐藤、今日の日付は？。<#3.0#>` | ラストシーン。冒頭と末尾に3秒の間 |

#### 沈黙（`<#秒数#>`）の設計パターン

| パターン | 例 | 用途 |
|---|---|---|
| 冒頭の間 | `<#2.9#>自分に関係ないと…` | 相手の台詞終了後の「考えている間」を表現 |
| 末尾の間 | `…ごまかしてるだけだろ。<#3.0#>` | 相手の返答を待つ間。10秒動画の尺合わせにも使用 |
| 途中の間 | `ごめん。<#1.3#>何か話して…` | 言葉に詰まる感情表現 |
| 長い間（7秒+） | `…調子が悪いのですが。<#7.0#>` | シーン転換・沈黙の演出 |

> **重要**: `<#秒数#>` の秒数は、事例4（順逆動画リップシンク手法）の計算に基づいて設計されている。10秒の動画に対してセリフ＋沈黙の合計がぴったり10秒になるよう調整する。
