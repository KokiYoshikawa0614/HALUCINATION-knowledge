...
| 2 | 黒塗り表現（LoRA→ChatGPT）| `pair2_before_lora_black_paint_partial_fail.png` | `pair2_after_chatgpt_black_face.png` | LoRAでは黒顔表現が不可能→ChatGPTに切り替えて解決 |
| 3 | LoRA崩壊（lora_scale）| `pair3_before_lora_correct_face.png` | `pair3_after_lora_breakdown_wrong_person.png` | lora_scale < 0.75 でプラチナブロンド長髪の西洋人に完全崩壊 |
...
## pair2: 黒塗り表現 — LoRA断念 → ChatGPTで解決

LoRAの「クリーン画像への過学習」により、顔を黒く覆う演出がLoRAでは実現不可能だった事例。

| Before（失敗）| After（解決）|
|---|---|
| ![pair2 before](pair2_before_lora_black_paint_partial_fail.png) | ![pair2 after](pair2_after_chatgpt_black_face.png) |
| 顎・首周りのみ黒くなり上半分はきれいなまま。白シャツも汚れなし | ChatGPTで生成。顔全体と上半身が厚く乾いた黒ペンキで覆われた表現を実現 |
...
**結論**: LoRAはクリーン画像に過学習しており、プロンプト語彙の変更だけでは根本解決に至らない。ツール切り替え（LoRA → ChatGPT）で解決した。後処理はTopaz（sharpen + denoise + upscale 3840w）。

---

## pair3: LoRA崩壊 — lora_scale の影響

lora_scale を下げた際に顔が完全に別人に崩壊した事例。Gemini LoRAプロンプト編集セッション中のスクリーンショットより。

| Before（正常）| After（崩壊）|
|---|---|
| ![pair3 before](pair3_before_lora_correct_face.png) | ![pair3 after](pair3_after_lora_breakdown_wrong_person.png) |
| lora_scale=0.85〜1.0。黒髪・黒スーツ・黒シャツのオールブラック。主人公の顔が正確に再現 | 同じLoRAでlora_scaleを下げた結果。プラチナブロンドの長髪・西洋人顔に完全崩壊 |

**教訓**: このLoRAは lora_scale=0.75 未満で別人が出現。使用可能範囲は 0.85〜1.0 のみ。
詳細は `02_prompts/replicate-params.md` を参照。
**画像ソース**: `raw-data/gemini-chats/LoRaプロンプト編集とシーン作成/`
