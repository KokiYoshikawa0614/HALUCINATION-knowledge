...
// Images used in the HTML (relative to repo root)
const IMAGE_PATHS = [
  'raw-data/正式ポスター.jpg',
...
(async () => {
  ...
  for (let i = 0; i < IMAGE_PATHS.length; i++) {
    ...
    if (metadata.width > MAX_WIDTH) {
      ...
      console.log(`  [${i}] ${path.basename(IMAGE_PATHS[i])}: ${origKB}KB -> ${newKB}KB`);
    } else {
      await sharp(srcPath)
        .jpeg({ quality: QUALITY })
        .toFile(outPath);
      console.log(`  [${i}] ${path.basename(IMAGE_PATHS[i])}: kept (${metadata.width}px)`);
    }
  }

  // Read HTML and replace image paths with resized versions
  let html = fs.readFileSync(path.resolve(__dirname, 'leadmagnet.html'), 'utf-8');

  for (let i = 0; i < IMAGE_PATHS.length; i++) {
    // Build the relative path as it appears in the HTML (../../raw-data/...)
    const relPath = '../../' + IMAGE_PATHS[i];
    const resizedRel = '_resized/img_' + i + '.jpg';
    html = html.split(relPath).join(resizedRel);
  }

  const compressedHtml = path.resolve(__dirname, '_leadmagnet_compressed.html');
  fs.writeFileSync(compressedHtml, html);

  // Generate PDF
  console.log('Generating PDF...');
  const browser = await puppeteer.launch();
  const page = await browser.newPage();
  await page.goto(`file://${compressedHtml}`, { waitUntil: 'networkidle0' });

  const pdfPath = path.resolve(__dirname, 'HALUCINATION_leadmagnet_compressed.pdf');
  await page.pdf({
    path: pdfPath,
    format: 'A4',
  ...
  console.log(`Done: HALUCINATION_leadmagnet_compressed.pdf (${sizeMB} MB)`);

  // Cleanup temp HTML
  fs.unlinkSync(compressedHtml);
})();
