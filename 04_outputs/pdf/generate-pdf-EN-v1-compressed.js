...
(async () => {
  ...
  let html = fs.readFileSync(path.resolve(__dirname, 'leadmagnet_en.html'), 'utf-8');
  for (let i = 0; i < IMAGE_PATHS.length; i++) {
    html = html.split('../../' + IMAGE_PATHS[i]).join('_resized/img_' + i + '.jpg');
  }

  const tmpHtml = path.resolve(__dirname, '_tmp_en_v1.html');
  ...
  const pdfPath = path.resolve(__dirname, 'HALUCINATION_leadmagnet_EN_v1_compressed.pdf');
  await page.pdf({
    path: pdfPath,
    format: 'A4',
  ...
  console.log(`Done: HALUCINATION_leadmagnet_EN_v1_compressed.pdf (${sizeMB} MB)`);
