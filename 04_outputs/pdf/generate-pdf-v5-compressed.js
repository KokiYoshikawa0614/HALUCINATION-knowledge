...
(async () => {
  ...
  for (let i = 0; i < IMAGE_PATHS.length; i++) {
    ...
    if (metadata.width > MAX_WIDTH) {
      ...
      console.log(`  [${i}] ${origKB}KB -> ${newKB}KB`);
    } else {
      await sharp(srcPath).jpeg({ quality: QUALITY }).toFile(outPath);
      console.log(`  [${i}] kept (${metadata.width}px)`);
    }
  }
  ...
  const tmpHtml = path.resolve(__dirname, '_tmp_v5.html');
  ...
  const pdfPath = path.resolve(__dirname, 'HALUCINATION_leadmagnet_v5_compressed.pdf');
  await page.pdf({
    path: pdfPath,
    format: 'A4',
  ...
  console.log(`Done: HALUCINATION_leadmagnet_v5_compressed.pdf (${sizeMB} MB)`);
